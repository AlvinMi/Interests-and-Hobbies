...
u8 MoniI2c_WriteByteToSlave(u8 addr,u8 reg,u8 data)
{
	...
	I2C_Start();                                    //产生起始信号
	state = MoniI2C_WriteByte(addr<<1|0);           //将从机地址和写方向写入总
...
/************************************************************
往从机中的寄存器写入多个数据，注意连续写入时，寄存器地址会自动增加
*addr：器件的地址
*reg：首个写入数据的寄存器地址
*len：要写入的数据的个数
*buf：要写入的数据区的首地址
...
/************************************************************************
从从机中读取采样到的高8位和低8位的数据,注意MPU6050是大端地址，并且注意：
在连续读取中，MPU6050寄存器地址会自增，
