...
struct hal_s {
    unsigned char sensors;				//用于指示用到了哪些传感器（如陀螺仪，加速计，磁力计）
    unsigned char dmp_on;				//用于指示开启/关闭DMP
    unsigned char wait_for_tap;			//等待手势操作，或者敲击指令
...
//    hal.report = PRINT_QUAT;									
    ...
    if(dmp_load_motion_driver_firmware())											//加载DMP固件
		return 5;
    if(dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation)))		//确定DMP姿态演算的坐标系
		return 6;
//    dmp_register_tap_cb(tap_cb);													//注册手势，点击等事件
//    dmp_register_android_orient_cb(android_orient_cb);							//注册安卓设备方向事件
    ...
    if(dmp_enable_feature(hal.dmp_features))										//使能要使用到的DMP的特性功能
		return 7;
    if(dmp_set_fifo_rate(DEFAULT_MPU_HZ))											//配置DMP输出采样频率，这里为100Hz（这里才是真正决定DMP采样频率的）
		return 8;
	
	
    mpu_set_dmp_state(1);			//开启dmp功能，并且会将MPU采样率和DMP输出采样率设置一致，同时打开DMP完成中断（中断模式默认被重新配置为：高有效，）
...
short sensors;								//sensors指示从FIFO中读取的数据是什么数据（四元数，陀螺仪，加速计等）
unsigned char more;							//指示FIFO中是否还有剩余的数据
long quat[4];								//四元数
extern MeasuredValue_s MV;
extern RawData_MPU_s RawData;				//gyro[3], accel[3]原始数据保存区
...
//	}
	...
	if (hal.new_gyro && hal.dmp_on) {
		 ...
		 */
		if(dmp_read_fifo(RawData.gyro, RawData.accel, quat, &sensor_timestamp, &sensors,&more)){		//开始从FIFO中读取数据
			return 1;
		}
...
//			send_packet(PACKET_TYPE_ACCEL, accel);
		...
		if(sensors & INV_XYZ_GYRO && hal.report & PRINT_GYRO   &&   sensors & INV_XYZ_ACCEL && hal.report & PRINT_ACCEL){
			ANO_DT_Send_Senser(RawData.accel[0],RawData.accel[1],RawData.accel[2],RawData.gyro[0],RawData.gyro[1],RawData.gyro[2],0,0,0,0);
		}
...
//			send_packet(PACKET_TYPE_QUAT, quat);
			...
			q2=quat[2] / q30;
			q3=quat[3] / q30;
			MV.pit  	= 	asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3;
			MV.rol 	= 	atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3; 
			MV.yaw 	= 	atan2(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * 57.3;
			
			//将姿态角数据上报给上位机
			if(hal.report & PRINT_QUAT)
				ANO_DT_Send_Status(MV.rol,MV.pit,MV.yaw,0,0,0);
		}
	...
	//未使用DMP，仅获取原始数据
		 ...
		 */
		if(mpu_read_fifo(RawData.gyro, RawData.accel, &sensor_timestamp, &sensors, &more)){				//仅从FIFO中获取原始数据
			//数据读取失败
			return 1;
		}
		if (!more)
