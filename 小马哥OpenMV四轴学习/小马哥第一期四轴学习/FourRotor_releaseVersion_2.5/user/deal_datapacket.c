...
//		DataPacket.yawSV = RxBuf[3]<<8|RxBuf[2];		//ADC2
		DataPacket.accelerator = RxBuf[5]<<8|RxBuf[4];	//ADC1
		DataPacket.leftRight = RxBuf[7]<<8|RxBuf[6];	//ADC4
		DataPacket.upDown = RxBuf[9]<<8|RxBuf[8];		//ADC3
		
//		printf("youmen:::%d\r\n",DataPacket.accelerator);
		
		//=======================采用指针直接操作内存的方式拼装数据================================
//		*((u8*)remoteControl) = RxBuf[2];
//		*(((u8*)remoteControl)+1) = RxBuf[3];
//		*(((u8*)remoteControl)+2) = RxBuf[4];
//		*(((u8*)remoteControl)+3) = RxBuf[5];
//		*(((u8*)remoteControl)+4) = RxBuf[6];
//		*(((u8*)remoteControl)+5) = RxBuf[7];
//		*(((u8*)remoteControl)+6) = RxBuf[8];
//		*(((u8*)remoteControl)+7) = RxBuf[9];

	}else if(RxBuf[0] & 0x08){							//当数据包是由遥控器的按键触发发送时
		DataPacket.buttonFlag = RxBuf[1];
	}
	
	//将数据包识别PID值取出，覆盖之前的值，以表示信号链接正常
	DataPacket.dataPID = RxBuf[10];
}
